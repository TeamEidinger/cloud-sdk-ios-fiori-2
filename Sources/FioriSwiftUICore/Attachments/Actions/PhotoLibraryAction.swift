...
public class PhotoLibraryAction<Attachment: AttachmentProcessing>: BaseAction<Attachment> {
    ...
    override func asButton(ctx: AttachmentActionContext<Attachment>) -> Button<Text> {
        Button(label) {
            ctx.dialogView = AnyView(ImagePicker<Attachment>(onSaveImage: self.onSaveImage))
